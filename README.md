...
# Chainboard Academy & Agoric - Lecture Seven Assignment, Lending Protocol
...
3. Checkout to latest feature branch `git checkout chainboard/lecture-seven`
...
4. Alice deposits `1 VAN` using the Dapp UI.
...
10. Check balances and activity tab.
  - APY for VAN Pool should go up

11. Accrue interest. Use `ManualTimer` for that.

    ```js
    // 8000 REPL
    // Remember to lookup 'TIMER_BOARD_ID' from lendingPoolDefaults.js file
    timer = E(home.board).get(TIMER_BOARD_ID)
    E(timer).tick()
    ```
...
21. Creator adds a new pool to the protocol.
    ...
    ```shell
    cd dapp-pool-lending-protocol
    agoric deploy api/addNewPool.js api/addHopToPool.js
    ```
